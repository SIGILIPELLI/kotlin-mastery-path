...
`build.gradle.kts` is not a config file that Gradle "reads" declaratively —
it is a real Kotlin script, compiled and executed by an embedded Kotlin
compiler on every build (with the compiled result cached so subsequent runs
skip recompiling an unchanged script). The `plugins { }`, `dependencies { }`,
and `application { }` blocks are ordinary Kotlin lambdas passed to functions
on the `Project` object, using Kotlin's trailing-lambda syntax you saw in
the lambdas module — `dependencies { implementation(...) }` is really
`dependencies(fun DependencyHandlerScope.() { implementation(...) })`, a
lambda with receiver, which is why you can call `implementation(...)` inside
the block with no explicit receiver: it's an extension-function-style
receiver being applied, exactly like the `this`-substitution mechanics
covered for extension functions.

`mainClass.set("com.example.MainKt")` is the same `<FileName>Kt` naming
convention from the very first module, now made explicit: if your `main()`
function lives directly in `Main.kt` with no wrapping object, the JVM entry
point Gradle needs to launch really is a class called `MainKt` generated by
the compiler, and this line tells the `application` plugin's packaging tasks
which class file to put in the runnable jar's manifest
(`Main-Class:` attribute) so `java -jar` knows where to start.

The Gradle wrapper works by having `gradlew` be a tiny, checked-in shell
script plus a `gradle-wrapper.properties` file recording an exact Gradle
distribution URL and version — the first invocation downloads and caches
that exact zip under `~/.gradle/wrapper/dists`, then every subsequent build
(on any machine, any CI runner) reuses that identical binary, which is how
"builds reproducibly across machines" becomes an actual guarantee rather
than a hope: the version is pinned in a file, not in whatever happens to be
on someone's `PATH`.
